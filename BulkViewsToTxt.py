...
def add_sql_metadata(database_name, object_name):
    return f"""
USE [{database_name}]
GO

/****** Object:  View [dbo].[{object_name}]    Script Date: {time.strftime('%d-%m-%Y %H:%M:%S')} ******/
SET ANSI_NULLS ON
GO
...
def read_and_create_txt_files(conn, database_name, object_type):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(f"""SELECT name FROM sys.views""")
    ...
    if conn_flag == 1:
        ...
        for database_name in databases:
            try:
                ...
                db_flag = read_and_create_txt_files(conn, database_name, 'Views')
                if db_flag == 1:
                    overall_flag = 1
